...
class _RolloutDataset(torch.utils.data.Dataset): # pylint: disable=too-few-public-methods
    ...
    def load_next_buffer(self):
        ...
        for f in self._buffer_fnames:
            with np.load(f) as data:
                ...
                tmp['observations'] =data['image_list']
                tmp['labels'] =data['label_list']
                ...
                self._cum_size += [self._cum_size[-1] +
                                   self._data_per_sequence(data['label_list'].shape[0])]
            pbar.update(1)
        pbar.close()
    ...
    def __getitem__(self, i):
        ...
        while True:
            seq_index=np.random.randint(0,len(data['observations']))


            if seq_index+self.leng > len(data['observations'])-1:
                seq_index = len(data['observations'])-1-self.leng
            safes = data['labels'][0 ][seq_index+self.leng]
            # print(data['labels'][0 ])
            # print(safes)
            #
            # i=number
...
class RolloutObservationDataset(_RolloutDataset): # pylint: disable=too-few-public-methods
    ...
    def _get_data(self, data, seq_index):
        return (data['observations'][seq_index],data['labels'][0][seq_index+self.leng])
